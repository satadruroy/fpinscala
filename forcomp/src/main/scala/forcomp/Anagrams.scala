...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c=>c).toList.sortWith(_._1 < _._1).map(e => (e._1, e._2.length))
  ...
  //  val pairs = for (w <- dictionary) yield (wordOccurrences(w), w)
  //  pairs.groupBy(_._1).mapValues(_.map(_._2)).withDefaultValue(List[Word]()) }
