...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def gt(tw1: Tweet, tw2: Tweet): Tweet = if (tw1.retweets > tw2.retweets) tw1 else tw2
    ...
    else if (left.isEmpty) gt(elem, rightmost)

    else if (right.isEmpty) gt(elem, leftmost)

    else gt(leftmost, gt(elem, rightmost))
